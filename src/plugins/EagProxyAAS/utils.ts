...
const authLogger = new Logger("MicrosoftAuth");
      ...
      if (!metadata || !metadata.session) {
        const quit = { quit: false },
          ...
          codeCallback = (code: ServerDeviceCodeResponse) => {
            ...
            authLogger.info(`Player ${client.gameClient.username} needs to authenticate: go to ${code.verification_uri} and enter code ${code.user_code}`);
          };
